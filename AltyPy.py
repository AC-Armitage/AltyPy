...
class Altypasser: 
    def __init__(self):
        ...
        #self.gui.iconbitmap("/home/lolmeh/Downloads/images.ico")
        self.infoframe = ctk.CTkFrame(master=self.gui)
        self.infoframe.pack(side="left", padx=5, fill="y")
        Image = tk.PhotoImage(file="/home/lolmeh/Downloads/azta/images.png")
        ...
        version = ctk.CTkLabel(master=self.infoframe, text="V0.9/11")
    ...
    def passer(self):
            ...
            exernumlabel = ctk.CTkLabel(master=pasframe, text="Approximately how many Lessons do you want to do:")
            ...
            def start():
                ...
                global progress_bar
                def add_prog():
                    ttdones.set(ttdones.get() + 1)
                ttdones = ctk.DoubleVar()
                ttdones.set(0)
                target = int(exernumtry.get())
                progress_bar = ttk.Progressbar(master=self.gui, variable=ttdones, maximum=target, length=400)
                progress_bar.pack(pady=20)
                eventcon = tk.Text(master= self.gui, fg="#60b389",bg="#16181c", highlightthickness=0, font=("Courier", 10))
                ...
                try:
                    ...
                    tk.messagebox.showwarning("Could not connect", "No connectivity or bad user credentials close the program and try again!")
                    sys.exit()
                                                    ...
                                                    try:
                                                        ...
                                                        while retries > 0:
                                                            ...
                                                            if exfire.status_code == 200:
                                                                ...
                                                                eventcon.see("end")
                                                                add_prog()
                                                                dones = int(ttdones.get())
                                                                eventcon.insert('end',f"[OK]\nEx successfully terminated [+] {dones}/{target}\n")
                                                                break
                                                            else:
                                                                print("Error")
                                                                eventcon.see("end")
                                                    ...
                                                    try:
                                                        ...
                                                        while retries > 0:
                                                            ...
                                                            if vfire.status_code == 200:
                                                                ...
                                                                add_prog()
                                                                dones = int(ttdones.get())
                                                                eventcon.insert('end',f"[OK]\nVideo successfully terminated [+]{dones}/{target}\n")
                                                                break
                                                            else:
                                                                print("Error")
                                                                eventcon.see("end")
    ...
    def timeadd(self):
            ...
            def start():
                    global stoptimebtn
                    global clockframe
                    global newclockframe
                    global pb
                    try:
                        pb = ctk.CTkProgressBar(self.gui, mode="indeterminate")
                        pb.pack(pady=10)
                        pb.start()
                                ...
                                if killer.status_code == 200:
                                    ...
                                    if newtimedem.status_code == 200:
                                        ...
                                        if timeadded >= timelimit:
                                            print("X+X+X+X+X+X+ I guess thats it +X+X+X+X+X+X\nAdios\nExiting program...")
                                            tk.messagebox.showwarning("Done", "Time have been added!")
                                            sys.exit()
                                            ...
                                            newtimeptxt.insert('end', f'{newhours}:{newmin}\n')
                                            newclockframe.update()
                                            pass
                                    else:
                                        print("Error fetching time ignoring")
                                        tk.messagebox.showwarning("Error", "Error fetching time ignoring")
                                        pass
                                    pass
                                else:
                                    print("Dead Error")
                                    tk.messagebox.showwarning("Error", "Unknown error")
                                pass
                            else:
                                print("Error: maybe you entered some wrong information! ")
                                tk.messagebox.showwarning("Error","Maybe you entered some wrong information!")
                                pass
                        except requests.exceptions.ConnectionError:
                            print("Slow or no connection")
                            tk.messagebox.showwarning("Error","Slow or no connection")
                            continue
            def starthh():
                startbutt.pack_forget()
                threading.Thread(target=start).start()
                return
            stop_time = False 
            ...
            timeadentry = ctk.CTkEntry(master=timeaframe, placeholder_text="In hours")
